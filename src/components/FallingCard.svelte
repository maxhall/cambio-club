...
<g transform="rotate({$tween.r*360},{$tween.x + width / 2},{$tween.y + width * 1.4 / 2}) translate({$tween.x} {$tween.y})">
	<rect
		width="100"
		height="140"
		rx="5"
	...
	></rect>
	<g transform="scale(0.2)" fill="{cardPathFill}" stroke="{(suit === "hearts" || suit === "diamonds") ? 'red' : 'black'}" stroke-width="{(stroked) ? 7.5 : 0}">
		<CardPath {rank} {suit} />
	</g>
</g>